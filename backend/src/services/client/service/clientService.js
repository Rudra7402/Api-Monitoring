...
const createNewApiKey = async (clientId, keyData, user) => {
    ...
    try {
        ...
        const shouldCreateReadKey = permissions.canReadAnalytics ?? true;
        if (shouldCreateReadKey) {
            const existingKeys = await findApiKeysByClientId(clientId);
            const existingReadKey = existingKeys.find((key) => key.permissions?.canReadAnalytics);
...
const getClientApiKeys = async (clientId, user) => {
    ...
    try {
        ...
        const formattedResponse = apiKeys.map((key) => key.toObject());
...
// Get a single client profile for an authorized user
const getClientProfile = async (clientId, user) => {
    try {
        if (!canUserAccessClient(user, clientId)) {
            throw new Error("Access denied");
        }
...
const updateClientDetails = async (clientId, updateData, adminUser) => {
    try {
        ...
        // Allowed fields only
        const allowedUpdates = adminUser.role === APPLICATION_ROLES.SUPER_ADMIN
...
// Update user role
const updateUserRole = async (clientId, userId, newRole, adminUser) => {
    try {
        // Access check
        if (!canUserAccessClient(adminUser, clientId)) {
            throw new Error("Access denied");
        ...
        if (adminUser.role !== APPLICATION_ROLES.SUPER_ADMIN &&
            adminUser.role !== APPLICATION_ROLES.CLIENT_ADMIN) {
            throw new Error("Only admins can update user role");
        }

        // Validate role
        if (!isValidClientRole(newRole)) {
            throw new Error("Invalid role for client user");
        }
        ...
        if (newRole === APPLICATION_ROLES.CLIENT_ADMIN) {
            permissions = {
                canCreateApiKeys: true,
                canManageUsers: true,
                canViewAnalytics: true,
        ...
        const updatedUser = await userRepository.updateUser(userId, { role: newRole, permissions });
        return formatClientUserResponse(updatedUser);
